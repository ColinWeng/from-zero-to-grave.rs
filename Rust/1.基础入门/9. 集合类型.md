...
fn main() {
    // 通过特征对象实现
    // 指定类型：`Vec<Box<dyn IpAddr>>`
    let v: Vec<Box<dyn IpAddr>> = vec![
        // 用 `Box::new` 包裹后，存在了数组 `v` 中
...
```rust
// 排序需要我们实现 `Ord` 特性，实现 `Ord` 需要我们实现 `Ord`、`Eq`、`PartialEq`、`PartialOrd` 这些属性。
...
fn main() {
    ...
    // `name` 是 `String` 类型，因此它受到所有权的限制，在 `insert` 时，它的所有权被转移给 `handsome_boys`
...
> 如果你使用**引用类型放入 HashMap 中**，请确保**该引用的生命周期至少跟 `HashMap` 活得一样久**。
