...
fn main() {
    // 使用 `std::net` 模块监听进入的请求连接，IP和端口是 `127.0.0.1:7878` 。
...
```shell
xxxxxxxxxx13 1$ cargo check2    Checking hello v0.1.0 (file:///projects/hello)3error[E0382]: use of moved value: `receiver`4  --> src/lib.rs:26:425   |621 |         let (sender, receiver) = mpsc::channel();7   |                      -------- move occurs because `receiver` has type `std::sync::mpsc::Receiver<Job>`, which does not implement the `Copy` trait8...926 |             workers.push(Worker::new(id, receiver));10   |                                          ^^^^^^^^ value moved here, in previous iteration of loop1112For more information about this error, try `rustc --explain E0382`.13error: could not compile `hello` due to previous error	
```
