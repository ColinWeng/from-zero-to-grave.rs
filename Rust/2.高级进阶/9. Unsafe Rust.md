...
​	至此，我们已经学过三种类似指针的概念：**引用**、**智能指针**和**裸指针**。与前两者不同，裸指针：
...
// 基于值的引用同时创建了不可变和可变的裸指针
// `as` 可以用于强制类型转换，将引用 `&num / &mut num` 强转为相应的裸指针 `*const i32 / *mut i32`。
...
fn split_at_mut(slice: &mut [i32], mid: usize) -> (&mut [i32], &mut [i32]) {
    ...
    // 试图在自定义的 `split_at_mut` 函数中，可变借用 `slice` 两次：
...
fn split_at_mut(slice: &mut [i32], mid: usize) -> (&mut [i32], &mut [i32]) {
    ...
    unsafe {
        (
            ...
            // 由于切片中的元素是 `i32` 类型，每个元素都占用了 4 个字节的内存大小，
            // 因此我们不能简单的用 `ptr + mid` 来作为初始地址，而应该使用 `ptr + 4 * mid`，但是这种使用方式并不安全，
            // 因此 `.add` 方法是最佳选择
...
```rust
// C 语言的代码定义在了 `extern` 代码块中，
extern "C" {
    fn abs(input: i32) -> i32;
}

fn main() {
    //  而 `extern` 必须使用 `unsafe` 才能进行进行调用
    unsafe {
