...
- `#[derive]`，在之前多次见到的**派生宏**，可以为目标结构体或枚举派生指定的代码，例如 `Debug` 特征
...
​	而**宏就可以拥有可变数量的参数**，例如可以调用一个参数的 `println!("hello")`，也可以调用两个参数的 `println!("hello {}", name)`。
...
// 通过 `vec!` 创建的动态数组支持任何元素类型，也并没有限制数组的长度，如果使用函数，我们是无法做到这一点的。
...
#[macro_export]
// 宏的名称是 `vec`，而不是 `vec!`，后者的感叹号只在调用时才需要。
macro_rules! vec {
    // `vec` 的定义结构跟 `match` 表达式很像，但这里我们只有一个分支，
    // 其中包含一个模式 `( $( $x:expr ),* )`，跟模式相关联的代码就在 `=>` 之后。
    // 一旦模式成功匹配，那这段相关联的代码就会替换传入的源代码。
...
​	可能有同学会提问：我们在使用标准库 `vec!` 时也没有引入宏啊，那是因为 Rust 已经通过 [`std::prelude`](https://course.rs/appendix/prelude.html) 的方式为我们自动引入了。
...
```bash
## 宏所在的包名自然也有要求，必须以 `derive` 为后缀，对于 `hello_macro` 宏而言，包名就应该是 `hello_macro_derive`。
## 在之前创建的 `hello_macro` 项目根目录下，运行如下命令，创建一个单独的 `lib` 包:
...
​	第一种是将 `hello_macro_derive` 发布到 `crates.io` 或 `GitHub` 中，就像我们引用的其它依赖一样；
...
```toml
# 还需要在 `[lib]` 中将过程宏的开关开启 : `proc-macro = true`
[lib]
proc-macro = true

# 其中 `syn` 和 `quote` 依赖包都是定义过程宏所必需的
[dependencies]
syn = "1.0"
quote = "1.0"
```
...
// 当用户使用 `#[derive(HelloMacro)]` 标记了他的类型后，
// `hello_macro_derive` 函数就将被调用。
...
// 对于其它部分基本都是一致的，如包的引入、宏函数的签名、语法树构建等
